...
- RCI constructor lot archetypes are named templates tagged with `zoningType` and `densityBand`. Residential and industrial now cover every 2x2 through 8x8 footprint, including rectangles, with at least two templates per density band and footprint.
...
- RCI growth is gated by `Data/RCI/rci_tools.xml`: each constructor-enabled RCI type declares a desirability threshold and max density-per-tile points interpolated from current population. Runtime then multiplies that citywide cap by a local land-value factor. Current desirability is 100 when the candidate has XML road access through the transport cost map and 0 otherwise.
