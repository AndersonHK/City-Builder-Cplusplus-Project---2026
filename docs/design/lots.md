...
- RCI constructor lot archetypes are tagged with `zoningType` and cover residential and industrial 2- and 3-tile widths for depths 2 through 8, plus representative larger 4x4, 4x6, 4x8, and 8x8 lots.
- RCI growth is gated by `Data/RCI/rci_tools.xml`: each constructor-enabled RCI type declares a desirability threshold and max density-per-tile points interpolated from current population. Current desirability is 100 when the candidate has XML road access through the transport cost map and 0 otherwise.
