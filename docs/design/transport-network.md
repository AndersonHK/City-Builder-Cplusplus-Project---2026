...
- Authoritative cost-map nodes use `tile + totalTiles * (mode + modeCount * layer)`. `TransportRouter` maps active nodes into a separate compact graph and reusable worker scratch.
...
- `TransportCostMap::findPath` remains a uniform-cost/Dijkstra reference API. Production commutes use `TransportRouter`: resumable outward nearest-destination search for demand fill and admissible A* for point-to-point return checks and repairs.
...
- Production routing shares traffic travel costs with deterministic equal-cost ordering. Additive route-seed jitter remains only in the legacy reference API; it is disabled for shared routing.
- Demand fill resumes one outward frontier across accepted destinations, split demand, and failed evening checks. Independent return checks and repairs run through `TransportRoutingPool`; the simulation thread commits capacity and traffic in deterministic source order. The destination index avoids rescanning every employer when all compatible jobs are occupied.
...
- Shared directed fields and an exact region-overlay prototype are available as APIs. The overlay is not enabled in production commutes because measurements were unfavorable; hierarchy and future service consumers remain follow-up work. See the [implementation record](transport-routing-implementation-plan.md).
...
- Lot queries display accepted morning routes. Road queries retain local morning/evening traffic totals and display complete matching one-way routes. Immutable per-step clocks feed rounded, mode-colored arrow blocks, defaulting to 10 seconds of travel each. Clocks update on repricing, exclude stationary car departure cost, and remain owned by the published/query snapshots; see [renderer notes](renderer.md).
...
- [Routing implementation record](transport-routing-implementation-plan.md) describes the shipped engine and deferred experiments; [real-city measurements](transport-routing-real-city-benchmark.md) document performance acceptance. Superseded proposals are listed in the [documentation index](../README.md).
