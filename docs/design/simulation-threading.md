...
- The RCI constructor iterates XML-defined zones and their allowed RCI types, up to the XML-configured attempt count per `(zone, RCI type)` pair. Under-construction RCI capacity reserves the demand budget immediately so repeated ticks do not keep filling demand that is already being built.
