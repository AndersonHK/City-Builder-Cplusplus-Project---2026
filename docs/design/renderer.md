...
- Ground roads render in the tile pass from packed road-state bytes and generated road atlases. The ground-road upload path is `UpdateGroundRoadChunkTexture` (`City Builder/Renderer.cpp:1558`).
...
- Elevated roads use separate per-chunk instance buffers and rebuild lazily for visible stale chunks. They consume the same resolved road glyph, lane graphic, and divider masks through `BuildRoadChunkInstances` (`City Builder/Renderer.cpp:1345`).
...
- Elevated road instances carry the same base/arrow glyphs and packed lane-graphic/divider masks as instance attributes (`City Builder/Renderer.cpp:1370`).
- `Basic.shader` unpacks the lane graphic and divider masks in `applyRoadEdgeOverlays` (`City Builder/Basic.shader:131`).
