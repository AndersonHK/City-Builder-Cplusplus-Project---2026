...
- When adding a hotkey, update `HotkeyConfig`, `ApplyHotkey`, `Data/config.ini`, README controls, and any relevant design checks together. Current road tool hotkeys are `S` street, `R` road, `O` one-way, `V` avenue, and `H` highway. Current overlay hotkeys are `T` for traffic capacity and `L` for land value.
