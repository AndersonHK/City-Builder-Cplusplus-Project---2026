...
- Low-density residential RCI zoning: `RGB(0, 192, 0)`
...
- `normal_res_empty_64`: low-density residential smart grid, no existing roads.
- `normal_res_edge_street_64`: low-density residential smart grid against an existing edge street.
- `normal_res_existing_grid_64`: low-density residential smart grid over an existing street grid.
- `normal_ind_existing_avenue_64`: industrial smart grid with an avenue and existing RCI zoning.
- `normal_res_existing_highway_64`: low-density residential smart grid with highway blockers.
- `shift_res_existing_roads_64`: Shift low-density residential lots-only mode with existing streets/roads.
- `normal_res_existing_rci_and_blocker_64`: low-density residential smart grid over old RCI zoning with a hard occupied blocker.
- `normal_res_offset_grid_snap_64`: low-density residential smart grid near touching offset street corridors.
...
```powershell
& "Distributable\x64\Release\RciSmartGridSandboxTests.exe"
```

Regenerate missing input templates and overwrite expected BMPs from the current planner:

```powershell
& "Distributable\x64\Release\RciSmartGridSandboxTests.exe" --bless
```
